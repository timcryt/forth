( Факториал N! = 1 * 2 * .. * {N-1} * N рекурсивное определение )
: !       ( n -- n!)           DUP 1 > IF DUP 1 - ! * ELSE DROP 1 THEN ;

( Числа Фиббоначи F{n} = F{n-1} + F{n-2} итеративное определение )
: FIB     ( n -- fib{n} )      1 - 0 1 BEGIN DUP >R + R> SWAP ROT 1 - DUP NOT >R ROT ROT R> UNTIL NIP NIP ;

( Комплект слов для нахождения простых чисел от 2 до X включительно )
: SQR     ( x -- {x^2} )       DUP * ;
: BUF     ( a -- bool{a} )     NOT NOT ;
: CR      ( -- )               10 EMIT ; ( Для DOS и Windows лучше добавить 13 EMIT )
...
( Комплект слов для выведения треугольника Серпинского )
: CR      ( -- )               10 EMIT ; ( Для DOS и Windows лучше добавить 13 EMIT )
: SPACE   ( -- )               32 EMIT ;
